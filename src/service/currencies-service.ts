...
import { DEFAULT_CURRENCY_CODE, EUR_CURRENCY_CODE } from '../config/currencies';
import { CURRENCIES_SERVICE_URL } from '../config/currencies-service-url';
...
class CurrenciesService {
  ...
  async getCurrenciesList(): Promise<any> {
    const response = await axios.get(CURRENCIES_SERVICE_URL + '/currencies');
  ...
  async getCurrenciesRateByBase(base: string = EUR_CURRENCY_CODE): Promise<RateInterface> {
    const response: AxiosResponse<RateInterface> = await axios.get(CURRENCIES_SERVICE_URL + `/latest?base=${base}`);
    return response.data
  }
}
